...
function drawGanttDataPoints(series, opts, config, context) {
  var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
...
function drawXAxisGrid(opts, config, context) {
  var spacingValid = opts.width - 2 * config.padding - config.xAxisHeight - config.legendHeight;
  var eachSpacing = Math.floor(spacingValid / config.yAxisSplit);
  var yAxisTotalWidth = config.yAxisWidth + config.yAxisTitleWidth;
  var startX = config.padding + 0;//yAxisTotalWidth;
  var endX = opts.height - 1 * config.padding - config.xAxisHeight - config.legendHeight;

  var points = [];
  for (var i = 0; i < config.yAxisSplit; i++) {
    points.push(config.padding + yAxisTotalWidth + eachSpacing * i);
  }
  // points.push(config.padding + eachSpacing * config.yAxisSplit + 2);
  points.push(opts.width - 1*config.padding-9); 
  context.beginPath();
  context.setStrokeStyle(opts.yAxis.gridColor || "#cccccc");
  context.setLineWidth(1);
  points.forEach(function (item, index) {
    context.moveTo(item, startX);
    context.lineTo(item,endX);
  });
  context.closePath();
  context.stroke();
}
...
function drawYAxisGantt(series, opts, config, context) {
  if (opts.yAxis.disabled === true) {
    return;
  }
  ...
  rangesFormat = [' 7:00', ' 9:00', '11:00', '13:00', '15:00', '17:00', '19:00', '21:00', '23:00'];
  // var points = [];
  // for (var i = 0; i <= 8; i++) {
  //   points.push(config.padding + eachSpacing * i);
  // }
    ...
    switch (type) {
        ...
        case 'gantt':
          this.animationInstance = new Animation({
            ...
            duration: duration,
            onProcess: function onProcess(process) {
              drawYAxisGrid(opts, config, context);  //绘制网格
              drawXAxisGrid(opts, config, context);  //暂时绘制出网格

              var _drawColumnDataPoints = drawGanttDataPoints(series, opts, config, context, process),
                xAxisPoints = _drawColumnDataPoints.xAxisPoints,
                eachSpacing = _drawColumnDataPoints.eachSpacing;

              _this.chartData.xAxisPoints = xAxisPoints;
              _this.chartData.eachSpacing = eachSpacing;
              drawXAxis(categories, opts, config, context);  //绘制 x坐标轴上的内容
              drawLegend(opts.series, opts, config, context);  //图表的页脚说明信息
              // drawYAxis(series, opts, config, context);     //绘制 y坐标轴上的内容
              drawYAxisGantt(series, opts, config, context);
              drawCanvas(opts, context);
            },
            onAnimationFinish: function onAnimationFinish() {
              _this.event.trigger('renderComplete');
...
var Charts = function Charts(opts) {
    ...
    config$$1.yAxisSplit = opts.yAxisSplit || config$$1.yAxisSplit;
